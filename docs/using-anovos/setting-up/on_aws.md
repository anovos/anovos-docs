...
Clone the Anovos repository into your local environment using the following command:
...
- Step1 : Users can go to Create cluster - Advanced options and use the following software configuration:
...
- Arguments for main.py file - `s3://<s3-bucket>/configs.yaml emr`. Here 1st argument is the s3 bucket path of config file and 2nd argument is run_type. 2nd argument is optional as we have already given in main.py file.
- Choose Action on failure as per your own wish as whether you want to do continue/cancel and wait/terminate .
Once you fill all the details, you can click add and go to next page in Create Cluster -  Advanced Options
...
The image below shows an example of setting networking for reference.
...
In this example , we have set Instance type as r5.2xlarge with 1 instance count of master node type and 5 instance count of Core node type having On-demand Purchasing option.We can choose instance type and no of instances as per dataset size and jobs complexity.For more information about purchasing options, see the learn more about instance purchasing options.
...
Once Cluster Nodes and Instances set up, we can go to next page for providing cluster name and Tags.A tag consists of a case-sensitive key-value pair. Tags on EMR clusters are propagated to the underlying EC2 instances.
Here in this example,the below image shows tags for reference.Users can give as per their own task and requirements.

![Tags Setting](../../assets/aws_emr_images/image9.png)

Once general cluster Settings completed, we can move to Bootstapping Action section.Bootstrap actions are scripts that are executed during setup before Hadoop starts on every cluster node. You can use them to install software and customize your applications.
Here we are using bootstrap actions to installs all the packages that are required to run _Anovos_ on EMR.Give the s3 bucket path of this script in script location as shown in the example and then click on save to add this script.
